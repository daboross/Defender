package net.daboross.defender.graphics.text;
 ...
 * <P>
 * <code>napier@potatoland.org</code>
 *
 * @see GLImage
 */
public class GLApp {
    ...
    static Map<String, String> OpenGLextensions;       // will be populated by extensionExists()
    ...
    public boolean initDisplay() {
        ...
        viewportX = (DM.getWidth() - viewportW) / 2;
        viewportY = (DM.getHeight() - viewportH) / 2;
        return true;
    }
    ...
    public void initInput() {
        try {
            ...
            if (disableNativeCursor) {
                ...
                cursorX = DM.getWidth() / 2;
                cursorY = DM.getHeight() / 2;
            }
    ...
    public static float[] getWorldCoordsAtScreen(int x, int y) {
        ...
        float[] resultf = new float[3];
        unProject((float) x, (float) y, z, resultf);
        return resultf;
    }
    ...
    public static double getTimeInMillis() {
        ...
        return (((double) Sys.getTime()) / (double) ticksPerSecond) * 1000.0;
    }
    ...
    public static void updateTimer() {
        ...
        secondsSinceLastFrame = (Sys.getTime() - lastFrameTime) / (double) ticksPerSecond;
        lastFrameTime = Sys.getTime();
        // keep a moving average of frame elapsed times
        if (secondsSinceLastFrame < 1) {
            avgSecsPerFrame = ((avgSecsPerFrame * numToAvg) + secondsSinceLastFrame) / (numToAvg + 1D);
        }
    }
     ...
     * with current settings (light, material, depth, blend, etc.)
     * <BR>
     *
     * @see loadImage()
     * @see drawQuad()
    ...
    public static void screenShotRGB(int width, int height, String saveFilename) {
        ...
        for (int i = 0; i < pixels.length; i++) {
            ...
            pixels[i] =
                    ...
                    | ((framebytes.get(bindex + 2) & 0x000000FF));   // B
        }
    ...
    public static void destroyDisplayLists() {
        while (displayLists.size() > 0) {
            int displaylistID = displayLists.get(0).intValue();
